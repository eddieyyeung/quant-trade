# factor-store-injection Specification

## Purpose
本能力规定「一个需要市场数据的组件，从哪里拿到数据库」。

组件（因子、策略、回测引擎）SHALL 从调用方接收数据访问对象，SHALL NOT 自行打开一个默认库；注册表的取用接口以统一形状透传该对象（`get(name, store=...)`），因子注册表与策略注册表一致，并以关键字传递。

未收到数据访问对象时，兜底构造 SHALL 解析**配置指定的**数据库路径而非硬编码字面量，并 SHALL 留下一条含该路径的警告——本能力存在的理由正在于此：漏传的后果是数据来源错误而非异常，没有痕迹就无从发现。兜底 SHALL NOT 因告警而中断调用；但配置本身无法解析时 SHALL 抛出，而不是另选一个与配置无关的库。

本能力只约束数据**从哪来**，不约束组件拿它算什么——因子的算法归 `factor-system`，选股逻辑归 `strategy-engine`。
## Requirements
