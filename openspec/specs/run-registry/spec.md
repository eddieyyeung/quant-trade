...
## Purpose
运行记录的持久化模型与生命周期：`run` 表保存每次运行的参数、状态与时间戳，状态机沿 `pending → running → ok / failed / cancelled / interrupted` 单向流转；运行日志写入 `run_log`，产物由 `artifact` 表索引。`trigger` 与 `idempotency_key` 作为预留字段本期即写入，使后续引入调度器与自动模拟盘时无需迁移历史数据。
## Requirements
### Requirement: 运行记录持久化
