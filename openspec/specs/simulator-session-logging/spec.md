...
模拟器一次会话可能跑几十周、跨越多个模块，出问题时需要能从日志还原它当时在做什么。

它规定各层在关键位置的日志义务：API 层记录会话创建请求与耗时结果，引擎层记录关键步骤进度，持久层记录子步骤细节，快照构建记录子模块与逐因子的进度。

每一条都同时约束「记什么」与「记到什么程度」——目的是让一次失败可以被定位到具体是哪一周、哪一个模块、哪一个因子，而不必复现。
