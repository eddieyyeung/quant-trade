# strategy-signal-persistence Specification

## Purpose
策略信号落库与服务层读取入口：`strategy_signal` 表一行一笔订单、以 `(run_id, seq)` 为主键且策略名冗余在行上，`run_strategy_signals` 在 `ctx.run_id` 非空时落库而既有入口与周报管线不受影响，并注册 `kind: strategy_signals` 任务类型、登记指向该表的产物记录；读取侧提供信号运行列表（服务端分页、以运行记录为主表）与单次运行的信号，查询不重跑信号生成，结果只增不改。
## Requirements
### Requirement: 策略信号表结构
