# model-evaluation-persistence Specification

## Purpose
模型评估结果持久化：以 `run_id` 为键把逐日 RankIC 序列、特征重要性与训练汇总指标写入 DuckDB 三张结果表，取消的训练保留已算出的部分，并提供运行列表、单次运行评估与指定日期预测的服务层读取入口，结果只增不改。
