# factor-ic-persistence Specification

## Purpose
因子 IC 的持久化与读取：`ic_series` 表以 `(factor_name, trade_date, forward_period)` 为主键存放逐日 IC / RankIC 与样本数，提供序列与汇总指标读取接口，配套批量 IC 计算函数与 `factor_ic` 计算任务。
## Requirements
